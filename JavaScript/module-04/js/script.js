...
function Cashier(name, productsDatabase) {
    ...
    this.onSuccess = function() {
        console.log(this.changeAmount > 0 ? `Спасибо за покупку, ваша сдача ${this.changeAmount})` : 'Спасибо за покупку');
    };
    ...
    this.countTotalPrice = function(order) {
        // метод, получает список покупок, считает общую стоимость исходя из
        // поля productsDatabase. Записывает результат в поле totalPrice.
        let orderTotal = 0;
        if (order && Object.keys(order).length > 0) {
            for (let item of Object.keys(order)) {
                orderTotal += order[item] * this.productsDatabase[item];
            }
        }
