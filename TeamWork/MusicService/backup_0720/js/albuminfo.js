...
// for (let prop in data.album)
//     console.log(prop);

// console.log('tags', data.album.tags);

// console.log(data.album.artist);
// console.log(data.album.name);
// console.log((new Date(data.album.wiki.published)).getFullYear());
// console.log(data.album.tracks.track);
console.log(data.album.image[2]['#text']);
// for (let track of data.album.tracks.track)
//     console.log(track.name, ' / ', track.duration, ' / ', Math.floor(track.duration / 60)+':'+(track.duration % 60));


// set up Handlebars helpers and partials
Handlebars.registerHelper('albumCover', (imgs) => imgs[2]['#text']);
Handlebars.registerHelper('albumYear', (strDate) => new Date(strDate).getFullYear());
Handlebars.registerHelper('trackDuration', (durationSec) => {
    let mm = Math.floor(+durationSec / 60);
    let ss = +durationSec % 60;
    return mm + ':' + (ss < 10 ? '0' : '') + ss;
});

// read data
const source = document.querySelector('#album-template').innerHTML.trim();
